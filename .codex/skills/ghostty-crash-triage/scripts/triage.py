...
@dataclass
class MinidumpValidation:
    is_valid: bool
    reason: str
    signature: str
    stream_count: int
    stream_directory_rva: int
    nonzero_directory_entries: int
    has_system_info_stream: bool
    minidump_size: int
...
def validate_minidump(minidump_path: Path) -> MinidumpValidation:
    """
    Validate basic minidump structure before invoking LLDB.

    This catches corrupt sparse files where the stream directory is all zeros.
    """
    if not minidump_path.exists():
        return MinidumpValidation(
            is_valid=False,
            reason=f"minidump attachment missing: {minidump_path.name}",
            signature="",
            stream_count=0,
            stream_directory_rva=0,
            nonzero_directory_entries=0,
            has_system_info_stream=False,
            minidump_size=0,
        )

    try:
        data = minidump_path.read_bytes()
    except OSError as err:
        return MinidumpValidation(
            is_valid=False,
            reason=f"failed to read minidump: {err}",
            signature="",
            stream_count=0,
            stream_directory_rva=0,
            nonzero_directory_entries=0,
    ...
    min_size = 32  # MDRawHeader is 32 bytes
    if len(data) < min_size:
        return MinidumpValidation(
            is_valid=False,
            reason=f"file too small for minidump header ({len(data)} bytes)",
            signature="",
            stream_count=0,
            stream_directory_rva=0,
            nonzero_directory_entries=0,
    ...
    signature_raw, _version, stream_count, stream_directory_rva = struct.unpack_from("<4sIII", data, 0)
    signature = signature_raw.decode("ascii", errors="replace")
    if signature != "MDMP":
        return MinidumpValidation(
            is_valid=False,
            reason=f"invalid header signature: {signature!r}",
            signature=signature,
    ...
    directory_size = stream_count * 12  # sizeof(MDRawDirectory)
    directory_end = stream_directory_rva + directory_size
    if stream_count <= 0:
        return MinidumpValidation(
            is_valid=False,
            reason="stream_count is zero",
            signature=signature,
            stream_count=stream_count,
            stream_directory_rva=stream_directory_rva,
            nonzero_directory_entries=0,
    ...
    if directory_end > len(data):
        return MinidumpValidation(
            is_valid=False,
            reason=(
                "stream directory points outside file "
                f"(dir_rva=0x{stream_directory_rva:x}, count={stream_count}, size={len(data)})"
            ),
            signature=signature,
            stream_count=stream_count,
            stream_directory_rva=stream_directory_rva,
    ...
    for i in range(stream_count):
        entry_offset = stream_directory_rva + (i * 12)
        stream_type, data_size, rva = struct.unpack_from("<III", data, entry_offset)
        if stream_type != 0 or data_size != 0 or rva != 0:
            nonzero_directory_entries += 1
        if stream_type == 7 and data_size > 0 and rva > 0:
            has_system_info_stream = True

    if nonzero_directory_entries == 0:
        return MinidumpValidation(
            is_valid=False,
            reason="stream directory entries are all zero (corrupt/incomplete minidump)",
            signature=signature,
            stream_count=stream_count,
            stream_directory_rva=stream_directory_rva,
            nonzero_directory_entries=nonzero_directory_entries,
            has_system_info_stream=has_system_info_stream,
            minidump_size=len(data),
        )

    if not has_system_info_stream:
        return MinidumpValidation(
            is_valid=False,
            reason="SYSTEM_INFO stream missing (LLDB cannot determine architecture)",
            signature=signature,
            stream_count=stream_count,
            stream_directory_rva=stream_directory_rva,
            nonzero_directory_entries=nonzero_directory_entries,
...
def write_minidump_validation(out_path: Path, validation: MinidumpValidation) -> None:
    """Persist minidump validation results for postmortem review."""
    out_path.write_text(
        json.dumps(
            {
                ...
                "stream_directory_rva": f"0x{validation.stream_directory_rva:x}",
                "nonzero_directory_entries": validation.nonzero_directory_entries,
                "has_system_info_stream": validation.has_system_info_stream,
                "minidump_size": validation.minidump_size,
            },
            indent=2,
            sort_keys=True,
        ),
...
def generate_report(
    ...
    minidump_validation: MinidumpValidation,
    has_lldb_bt: bool,
    has_atos: bool,
    files: list[str],
) -> Path:
    ...
    stack_todo = (
        ...
        if has_lldb_bt
        else "[TODO: Minidump is corrupt; stack unavailable. Investigate crash writer path.]"
    )
...
## Evidence
{"- Backtrace: `lldb_bt.txt`" if has_lldb_bt else "- Backtrace: unavailable (invalid minidump - see `minidump_validation.json`)"}
{"- atos fallback: `atos_bt.txt`" if has_atos else ""}
- Event summary: `event_summary.json`
- Minidump validation: `minidump_validation.json` ({minidump_validation.reason})
...
def triage(
    ...
    minidump_validation = validate_minidump(minidump_path)
    validation_path = out_dir / "minidump_validation.json"
    write_minidump_validation(validation_path, minidump_validation)
    print(f"minidump validation: {minidump_validation.reason}", flush=True)

    # 8. Run lldb
    lldb_bt_path = out_dir / "lldb_bt.txt"
    success = False

    if minidump_validation.is_valid:
        ...
        print("warning: skipping lldb symbolication due to invalid minidump structure", flush=True)

    # 9. Analyze symbolication
    total_frames, unsym_count, unsym_addrs = analyze_symbolication(lldb_bt_path)
    has_lldb_bt = lldb_bt_path.exists() and lldb_bt_path.stat().st_size > 0
    ...
    if has_lldb_bt:
        ...
        print("  1. Review minidump_validation.json for corruption details")
        print("  2. Edit REPORT.md with your analysis")
    print("=" * 60)
